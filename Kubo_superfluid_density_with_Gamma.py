# -*- coding: utf-8 -*-
"""
Created on Fri Jul 17 11:44:00 2026
...
q_B_constant = 3/(2*np.sqrt(gamma*mu))  
theta = np.pi/2
N_phi = 3   
cut_off = 2 * k_F  

# --- DISORDER & MATSUBARA PARAMETERS ---
eta = 2*0.005    # meV: Disorder scattering rate (quasiparticle lifetime broadening)
N_omega = 2000 # Number of fermionic Matsubara frequencies to sum over
beta = 200     # Inverse Temperature (1 / k_B T)
...
parameters = {"gamma": gamma, "points": points, "k_F": k_F,
              ...
              "cut_off": cut_off,
              "beta": beta, "eta": eta, "N_omega": N_omega,
              "k_values": k_values, "theta_values": theta_values,
...
def process_B_chunk(B_chunk):
    ...
    deriv_surface = np.pi * gamma * cut_off**2

    angular_integrand_x = []
    angular_integrand_y = []

    # Generate fermionic Matsubara frequencies: w_n = (2n + 1)*pi / beta
    omega_values = (2 * np.arange(N_omega) + 1) * np.pi / beta
    # Incorporate disorder self-energy directly into the Matsubara channels
    iw_values = 1j * omega_values + 1j * eta
    ...
    for t_val in theta_values:
        ...
        # Initialize containers to accumulate the Matsubara sum fields dynamically
        dia_term = np.zeros((N_B_chunk, len(k_values)))
        Kernel = np.zeros((N_B_chunk, len(k_values), 4, 4))
        Tau_diag = np.real(np.diagonal(Tau_tilde, axis1=2, axis2=3))

        # Stream memory-efficiently along the Matsubara frequency axis
        for iw in iw_values:
            # G_omega shape: (N_B_chunk, N_k, 4) -- Uses minimal RAM allocation space
            G_omega = 1.0 / (iw - E_tensor)
            
            # Accumulate diamagnetic weight elements
            dia_term += (2.0 / beta) * np.sum(np.real(G_omega) * Tau_diag, axis=2)
            
            # Accumulate paramagnetic bubble configurations
            for a in range(4):
                for b in range(4):
                    Kernel[:, :, a, b] += (2.0 / beta) * np.real(G_omega[:, :, a] * G_omega[:, :, b])
                
        para_term_x = np.sum(Kernel * np.abs(J_x_tilde)**2, axis=(2, 3))
        para_term_y = np.sum(Kernel * np.abs(J_y_tilde)**2, axis=(2, 3))
        
        integrand_x = 0.5 * (dia_term + para_term_x) * k_values[np.newaxis, :]
        integrand_y = 0.5 * (dia_term + para_term_y) * k_values[np.newaxis, :]
    ...
    n_xx_chunk = total_integral_x + deriv_surface
    n_yy_chunk = total_integral_y + deriv_surface
...
if __name__ == "__main__":
    ...
    name = f"superfluid_density_DisorderedKubo_Delta={Delta}_lambda={np.round(Lambda, 2)}_eta={eta}_points={points}_N={N}_beta={beta}_q_B_constant={q_B_constant}.npz"
