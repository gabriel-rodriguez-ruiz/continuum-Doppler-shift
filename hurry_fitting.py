...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 11:37:44 2026
...
# file_to_open = data_folder / "superfluid_density_B_in_1.6_(0.8-5.0)_phi_x_in_(-0.0-0.0)_Delta=0.08_lambda=0.64_points=16_N_phi=1_N=300_C=0.npz"
file_to_open = data_folder / "superfluid_density_with_Doppler_shift_B_in_1.6_(0.0-3.0)_phi_x_in_(-0.0-0.0)_Delta=0.08_lambda=0.64_points=19_N_phi=3_N=300_T=False_beta=150.npz"
...
# superfluid_density_xx = np.sqrt(Data["superfluid_density_xx"])
# superfluid_density_yy = np.sqrt(Data["superfluid_density_yy"])
superfluid_density_xx = Data["superfluid_density_xx"]
superfluid_density_yy = Data["superfluid_density_yy"]


fig, ax = plt.subplots()
ax.scatter(B_values/Delta, (superfluid_density_xx-superfluid_density_xx[0])/superfluid_density_xx[0], label=r"$D_{s}(B_{\perp}, \textbf{q}_B=0)$",
           s=40, marker="o")
ax.scatter(B_values/Delta, (superfluid_density_yy-superfluid_density_yy[0])/superfluid_density_yy[0], label=r"$D_{s}(B_{\perp}, \textbf{q}_B=0)$", s=40, marker="v")
...
B_c = field_0[9]   #field_0[9]   #data["field 0°"][14]# 0.07  T critical field
mu_B = 5.79e-2 # meV/T
...
x = B_values/Delta
axs[0].plot(x, interpolation_for_theory(x)[0], "o-")
axs[0].scatter(B_values/Delta, superfluid_density_xx, label=r"$D_{s}(B_{\perp}, \textbf{q}_B=0)$",
            facecolors='none', edgecolors='green', s=40, marker="v")
axs[1].plot(x*B_c, model_perpendicular(x, a=3.10460843e-05, c=0), "o-")
...
# file_to_open = data_folder / "superfluid_density_with_Doppler_shift_B_in_1.6_(0.0-3.0)_phi_x_in_(-0.0-0.0)_Delta=0.08_lambda=14.96_points=19_N_phi=3_N=100_T=True_beta=100_m=2.283666666666667e-29.npz"
...
ax.scatter(B_values/Delta, superfluid_density_yy, label=r"$D_{s}(B_{\parallel}, \mathbf{q}_B=0)$",
           facecolors='none', edgecolors='blue', s=40, marker="o")
...
B_c = field_0[14]   #field_0[9]   #data["field 0°"][14]# 0.07  T critical field
mu_B = 5.79e-2 # meV/T
g = 0.08 / (mu_B*B_c)  #Delta/(mu_B*B_c )   #1 / 1.7
g_xx = g
g_yy = g
...
scale_factor_perpendicular = 4.5
popt_perpendicular[0] = popt_parallel[0] 

popt_perpendicular[1] = popt_parallel[1]
...
ax.plot(B_values/Delta*B_c, scale_factor_perpendicular * ( (superfluid_density_xx-superfluid_density_xx[0])/(superfluid_density_xx[0]+popt_parallel[0]) + popt_parallel[1] * (B_values/Delta*B_c)**2),
        "--ko",  label=r"fit of $n_s(\gamma=0, \theta=90°)$")
ax.plot(B_perpendicular[:41], scale_factor_perpendicular *  model_perpendicular(x_model_perpendicular, popt_perpendicular[0], 0) + popt_parallel[1] * B_perpendicular[:41]**2, "-b",  label=r"fit of $n_s(\gamma=0, \theta=0°)$", zorder=3)
...
fitting = np.column_stack([B_values/Delta*B_c, popt_parallel[0]*(superfluid_density_yy_0-superfluid_density_yy_0[0])
        + popt_parallel[1]*(B_values/Delta*B_c)**2,
        popt_perpendicular[0]*(superfluid_density_finite_differences_perpendicular_0-superfluid_density_finite_differences_perpendicular_0[0])
                + popt_perpendicular[1]*(B_values/Delta*B_c)**2
        ])

filename = '5.8Ghz_field_dependence_fitting_at_78mK_06_03_26.dat'
np.savetxt(filename, fitting[:36, :])

plt.plot(fitting[:, 0], fitting[:, 1])
plt.plot(fitting[:, 0], fitting[:, 2])
plt.show()
...
ax.errorbar(field_45[:41], n_s_45[:41], yerr=n_s_45_error[:41], label=r"$n_s(45°)$", color="green", fmt="o")
# ax.errorbar(field_90, n_s_90, yerr=n_s_90_error, label=r"$n_s(90°)$", color="black", fmt="o")
ax.errorbar(field_135[:41], n_s_135[:41], yerr=n_s_135_error[:41], label=r"$n_s(135°)$", color="darkviolet", fmt="o")

from scipy.optimize import curve_fit

def interpolation_for_theory(x):
    return [
            np.interp(x, B_values/Delta, superfluid_density_finite_differences_perpendicular_0),        #I have change x to x/2
            np.interp(x, B_values/Delta, superfluid_density_yy_0),
            ]
...
B_c = field_0[8]   #data["field 0°"][14]# 0.07  T critical field
mu_B = 5.79e-2 # meV/T
g = 0.08 / (mu_B*B_c)  #Delta/(mu_B*B_c )   #1 / 1.7
g_xx = g
g_yy = g
...
x_model_perpendicular  = field_135[7:19]/B_c  #field_90[7:41]/B_c
...
initial_parameters_perpendicular = [ 3.73583079e+06, -5.40525133e+02]
popt_perpendicular, pcov_perpendicular = curve_fit(
                                                   model_perpendicular, x_model_perpendicular, n_s_135[7:19],
                                                   p0=initial_parameters_perpendicular
                                                   )

popt_perpendicular[0] = 2.4*popt_perpendicular[0]   # 1.7
c = popt_parallel[1]#-1
...
x_original = x_model_perpendicular[3:]
y_original = model_perpendicular(x_model_perpendicular[3:], *popt_perpendicular)
# cs = CubicSpline(x_original, y_original, bc_type=(1, 0), extrapolate="True")
f = interp1d(x_original, y_original, kind='zero', fill_value='extrapolate')
# cs.extend([x_original[10], x_model_perpendicular[18]]) # This modifies the 'cs' object in-place
x_new = B_perpendicular[10:41]
y_new = f(x_new/B_c)
ax.plot(x_new, y_new, '-', label='Extended Interpolation')
