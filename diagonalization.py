...
def get_Eigenvectors_in_polars(k_values, theta_values, mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y):
    """Return the energies of the Hamiltonian at a given k."""
    E = np.zeros((len(k_values), len(theta_values), 4), dtype=complex)
    V = np.zeros((len(k_values), len(theta_values), 4, 4), dtype=complex)
    for i, k in enumerate(k_values):
        for j, theta in enumerate(theta_values):
            H = get_Hamiltonian_in_polars(k, theta, mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y)
            E[i, j, :], V[i, j, :, :] = scipy.linalg.eigh(H)
    return E, V
...
def get_Energies_in_polars(k_values, theta_values, mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y):
    ...
    def eigh_interval(k, theta):
        ...
        e, psi = get_Eigenvectors_in_polars([k], [theta], mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y)
