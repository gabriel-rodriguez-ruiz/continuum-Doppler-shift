...
def integrate_phi_x(phi_x):
    ...
    # I add 1/2 because the 1/2 of BdG is asociated to the integration in k and not to the eigenenergies.
    fundamental_energy_2DEG = 1/2*energy_phi_2DEG +  np.pi/2 * cut_off**2 * (2*gamma*(phi_x+q_B)**2 - 2*mu + gamma*cut_off**2)
...
def integrate_phi_y(phi_y):
    ...
    energy_phi_2DEG = np.sum(integral) + np.sum(low_integral) + np.sum(high_integral)
    fundamental_energy_2DEG = 1/2*energy_phi_2DEG  + np.pi/2 * cut_off**2 * (2*gamma*phi_y**2 - 2*mu + gamma*cut_off**2)

    return fundamental_energy_2DEG, normal_density


if __name__ == "__main__":
    phi_x_values = np.linspace(-0.001, 0.001, points)  #phi_x_values = np.linspace(-0.0015, 0, points)
