...
k_values = np.append(np.linspace(0, 0.97*k_F, N), [np.linspace(0.97*k_F, 1.03*k_F, N),
            np.linspace(1.03*k_F, cut_off, N)])
...
def get_Hamiltonian_for_single_theta(k_1d, theta_val, mu, phi_x, gamma, Lambda, phi_y):
    """
...
def get_ground_state_energy(k_1d, theta_1d, mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y):
    ...
    for t_val in theta_1d:
        H_slice = get_Hamiltonian_for_single_theta(k_1d, t_val, mu, phi_x, gamma, Lambda, phi_y)
...
def get_ground_state_energy_vs_q_with_Doppler(k_1d, theta_1d, mu, B_y, Delta, q, gamma, Lambda, B_x, phi_y, q_D_x):
    q_x = q * np.cos(varphi) + q_D_x
    q_y = q * np.sin(varphi)
    E_q = get_ground_state_energy(k_1d, theta_1d, mu, B_y, Delta, q_x, gamma, Lambda, B_x, q_y)
    return E_q

def integrate_phi_x(phi_x):
    return get_ground_state_energy_vs_q_with_Doppler(k_values, theta_values, mu, B_y, Delta, phi_x, gamma, Lambda, B_x, phi_y, q_D_x)

if __name__ == "__main__":
    phi_x_values = np.linspace(-3*q_c, 3*q_c, points)
    
    # Fix for Windows multiprocessing environments
    # Ensures cleanly isolated process execution loops
    with multiprocessing.Pool(n_cores) as pool:
        fundamental_energy_2DEG = pool.map(integrate_phi_x, phi_x_values)
        
    fundamental_energy_2DEG = np.array(fundamental_energy_2DEG)
    data_folder = Path("Data/")
    data_folder.mkdir(exist_ok=True)
    
    name = f"total_fundamental_energy_B={B}_phi_x_in_({np.round(np.min(phi_x_values), 3)}-{np.round(np.max(phi_x_values),3)})_Delta={Delta}_lambda={np.round(Lambda, 2)}_points={points}_N_phi={N_phi}_N={N}_T={T}_beta={beta}_q_D_x={q_D_x}.npz"
    file_to_open = data_folder / name
    np.savez(file_to_open, fundamental_energy_2DEG=fundamental_energy_2DEG,
